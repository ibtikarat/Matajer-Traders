...
class ServicesClientsVC: UIViewController , WKNavigationDelegate, QLPreviewControllerDataSource {
    ...
    func loadPage(){
        var urlRequest = URLRequest(url: URL(string:API.DOMAIN_URL + "partnerServiceCategory")!)
        ...
        if keyPath == "estimatedProgress" {
            if self.webView.estimatedProgress != 1 {
                ...
                if let currentURL = self.webView.url?.absoluteString{
                    ...
                    print(currentURL)
                    if currentURL.description != "\(API.DOMAIN_URL)partnerServiceCategory" {
                        isMain = false
                    }else {
                        isMain = true
                    }
