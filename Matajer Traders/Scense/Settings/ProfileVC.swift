...
class ProfileVC: UIViewController , WKNavigationDelegate , QLPreviewControllerDataSource {
    ...
    func loadPage(){
        var urlRequest = URLRequest(url: URL(string:API.DOMAIN_URL + "profile/show")!)
        ...
        if keyPath == "estimatedProgress" {
            if self.webView.estimatedProgress != 1 {
                ...
                if let currentURL = self.webView.url?.absoluteString{
                    if currentURL.description != "\(API.DOMAIN_URL)profile/show" {
                        isMain = false
                    }else {
                        isMain = true
                    }
