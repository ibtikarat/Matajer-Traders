...
class CustomersVC: UIViewController , WKNavigationDelegate,QLPreviewControllerDataSource {
    ...
    override func viewDidAppear(_ animated: Bool) {
        ...
        NotificationCenter.default.addObserver(forName: NSNotification.Name(rawValue: "UpdateCustomers"), object: nil, queue: nil) { (notification) in
            self.loadPage()
        ...
        if keyPath == "estimatedProgress" {
            ...
            if self.webView.estimatedProgress != 1 {
                ...
                if let currentURL = self.webView.url?.absoluteString{
                  ...
                  if currentURL.description != "\(API.DOMAIN_URL)client" {
